...
const ConsultarIdNorte: React.FC = () => {
  ...
  const athlete = useMemo(() => {
    ...
    return db.users.find(u => u.idNorteNumero === numeroParam);
  }, [db.users, numeroParam]);

  const isValid = useMemo(() => {
    if (!athlete) return false;
    if (athlete.idNorteStatus !== 'APROVADO') return false;
    if (!athlete.idNorteValidade) return false;
    
    const validade = new Date(athlete.idNorteValidade);
          ...
          <div className="bg-slate-900 rounded-[40px] border border-slate-800 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className={`p-10 text-center space-y-6 ${isValid ? 'bg-emerald-500/5' : 'bg-red-500/5'}`}>
              ...
              <div>
                ...
                <h2 className="text-3xl font-black text-white mt-4 tracking-tight">{athlete.nomeCompleto}</h2>
              </div>
            </div>
            ...
            <div className="p-8 space-y-6">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-slate-800 rounded-2xl flex items-center justify-center border border-slate-700 overflow-hidden">
                   {athlete.fotoUrl ? <img src={athlete.fotoUrl} className="w-full h-full object-cover" /> : <UserIcon className="w-6 h-6 text-slate-600" />}
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Registro Oficial</p>
                  <p className="text-lg font-black text-emerald-500 font-mono">{athlete.idNorteNumero}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800">
                  ...
                  <p className="font-bold text-white text-sm">{formatDate(athlete.idNorteAdesao)}</p>
                </div>
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800">
                  <div className="flex items-center gap-2 text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">
                    <Calendar className="w-3 h-3" /> Validade
                  </div>
                  <p className="font-bold text-white text-sm">{formatDate(athlete.idNorteValidade)}</p>
                </div>
              </div>
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex items-center gap-3">
                 ...
                 <div>
                   ...
                   <p className="font-bold text-white text-sm">{athlete.cidade} - {athlete.estado}</p>
                 </div>
              </div>
            </div>

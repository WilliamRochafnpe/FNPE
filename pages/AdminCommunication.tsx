...
const AdminCommunication: React.FC = () => {
  ...
  const list = useMemo(() => {
    return db.users.filter(u => {
      const matchesSearch = u.nomeCompleto.toLowerCase().includes(searchTerm.toLowerCase()) || 
                           u.email.toLowerCase().includes(searchTerm.toLowerCase());
      const isTarget = activeTab === 'PESCADORES' ? u.idNorteStatus !== 'APROVADO' : u.idNorteStatus === 'APROVADO';
                ...
                list.map(u => (
                  ...
                  >
                    ...
                    <div className="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center text-slate-500 font-bold overflow-hidden">
                      {u.fotoUrl ? <img src={u.fotoUrl} className="w-full h-full object-cover" /> : u.nomeCompleto.charAt(0)}
                    </div>
                    <div className="flex-1">
                      <p className="font-bold text-slate-900 leading-none">{u.nomeCompleto}</p>
                      <p className="text-xs text-slate-400 mt-1">{u.email}</p>
                    </div>
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-lg ${
                      u.idNorteStatus === 'APROVADO' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
                    }`}>
                      {u.idNorteStatus === 'APROVADO' ? 'FILIADO' : 'PENDENTE'}
                    </span>
                  </div>
                ))
              )}

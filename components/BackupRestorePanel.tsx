...
const BackupRestorePanel: React.FC = () => {
  ...
  const handleRestoreSnapshot = (snap: Snapshot) => {
    if (!confirm(`Restaurar snapshot de ${new Date(snap.createdAt).toLocaleString()}? O estado atual será salvo em um novo snapshot.`)) return;

    // Snapshot do estado atual antes de restaurar
    createSnapshot(db, "Antes de restaurar snapshot");
    
    applyNewDB(snap.data, 'Sistema restaurado para o ponto selecionado.');
  };
              ...
              snapshots.map(snap => (
                <div key={snap.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 group">
                  <div className="overflow-hidden">
                    <p className="text-[10px] font-black text-slate-800 truncate">{snap.label || 'Sem nome'}</p>
                    <p className="text-[8px] text-slate-400 font-bold">{new Date(snap.createdAt).toLocaleString()}</p>
                  </div>
